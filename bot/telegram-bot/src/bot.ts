...
// Update fun facts
const funFacts = [
  "Did you know? ALBJ is a culturally inspired meme coin that fuses global folklore, ancestral traditions, dream-world symbology, and decentralized finance into a powerful new digital asset! 🌟",
  "Fun fact: ALBJ draws from the vibrant and inter-dimensional realm of spirit-creatures — beings that combine elemental and animal forms — bridging humanity's timeless myths with Web3! 🎨",
  "Interesting: Hybrid and chimera-like creatures have appeared throughout humanity's oldest civilizations, from Greek mythology's Chimera to Egyptian Sphinxes, Japanese Yōkai, and Mexican Alebrijes! 🌍",
  "Did you know? By tapping into universal archetypes, ALBJ becomes a global guide of imagination, resilience, and cultural storytelling for the blockchain era! ✨",
  ...
  "Fun fact: ALBJ combines traditional art with modern blockchain technology! 🌟",
...
// Add command to send message to a specific channel
bot.onText(/\/send (.+)/, async (msg: Message, match: RegExpExecArray | null) => {
  const chatId = msg.chat.id;
  if (!match) {
    bot.sendMessage(chatId, 'Please specify a channel and message. Example: /send announcements Hello world!');
    return;
  }

  const [_, channelAndMessage] = match;
  const [channel, ...messageParts] = channelAndMessage.split(' ');
  const message = messageParts.join(' ');

  const validChannels = ['announcements', 'rules', 'token-info', 'nft-preview', 'community-events', 'support'];
  
  if (!validChannels.includes(channel)) {
    bot.sendMessage(chatId, `Invalid channel. Please use one of: ${validChannels.join(', ')}`);
    return;
  }
  ...
  try {
    ...
    if (!channelId) {
      bot.sendMessage(chatId, `❌ Channel ID not found for ${channel}`);
      return;
    }

    await bot.sendMessage(channelId, message, { parse_mode: 'Markdown' });
    bot.sendMessage(chatId, `✅ Message sent to #${channel} channel!`);
  } catch (error) {
    console.error(`Error sending message to ${channel}:`, error);
    bot.sendMessage(chatId, `❌ Error sending message to ${channel}. Please try again.`);
  }
});